#!/usr/bin/env python3
"""
Batch evaluator for Qwen 2.5 72B - Test 100 queries with path guidance
"""
...
Generate SQL:"""
    ...
    def call_llm(self, prompt: str, query_idx: int) -> Optional[Dict]:
        ...
        # For Qwen, try to request logprobs even though they may not work
        data = {
            "model": self.model,
            "messages": [{"role": "user", "content": prompt}],
            "temperature": 0.1,
            "max_tokens": 600,
            "top_logprobs": 5  # This won't work but won't break either
        }
    ...
    def run_batch(self, batch_data: List[Dict], num_queries: int = 100):
        ...
        print(f"🚀 Qwen 2.5 72B Batch Evaluation: {num_queries} queries")
        print(f"🤖 Model: {MODEL}")
        print("=" * 80)
        ...
        print(f"\n🔍 LOGPROBS STATUS:")
        logprob_results = [r for r in self.results if r["status"] == "success"]
        has_logprobs = sum(1 for r in logprob_results if r.get("has_logprobs", False))
        print(f"   Queries with logprobs: {has_logprobs}/{len(logprob_results)}")
...
def main():
    ...
    # Run evaluation
    evaluator = QwenBatchEvaluator(bird_db_path)
